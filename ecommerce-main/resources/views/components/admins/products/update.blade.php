...
<div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <script>
    ...
    async function FillUpdateForm(id) {
        ...
        let res = await axios.post('/product-byId',{id:id})

        $('#nameUpdate').val(res.data['rows']['name'])
        $('#descriptionUpdate').val(res.data['rows']['description'])
        $('#stockUpdate').val(res.data['rows']['stock'])
        $('#priceUpdate').val(res.data['rows']['price'])
        $('#colorUpdate').val(res.data['rows']['color'])
        $('#categoryIdUpdate').val(res.data['rows']['category_id'])
        $('#brandIdUpdate').val(res.data['rows']['brand_id'])
        $('#imageInputUpdate').val(res.data['rows']['image'])
